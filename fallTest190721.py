	...
	try:
		...
		if(phaseChk <= 3):
			...
			while (tx2-tx1<=x):
				...
				Other.saveLog(releaseLog, GPS.readGPS(), BME280.bme280_read(), TSL2561.readLux(), BMX055.bmx055_read())
				time.sleep(1)
				Other.saveLog(releaseLog, GPS.readGPS(), BME280.bme280_read(), TSL2561.readLux(), BMX055.bmx055_read())
				time.sleep(1)
				tx2=time.time()
